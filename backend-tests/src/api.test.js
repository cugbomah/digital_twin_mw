...
//const baseUrl = `${apiUri}:${apiPort}/api/v1`;
const JWT_SECRET = process.env.JWT_SECRET || "digitalTwinningProject";
...
describe("dtmw api", () => {
  describe("User CRUD", () => {
    const userToCreate = {
      email: `testuser${Date.now()}@user.com`,
    ...
    const ownerToCreate = {
      email: `testowner${Date.now()}@owner.com`,
      firstName: `FirstName${Date.now()}`,
      lastName: `LastName${Date.now()}`,
      password: `${Date.now()}`,
    };
    ...
    test("owner sign up", async () => {
      const { data: response } = await axios.post(
        `${baseUrl}/owners/signup`,
        //myUrl,
        ownerToCreate
      );
      expect(response).toHaveProperty("data");
      const { data } = response;
      checkLoggedInUser(data, ownerToCreate);
      const createdOwner = await db
        .table("core_user")
        .where("email", ownerToCreate.email)
        .first();
      expect(createdOwner.id).toBe(data.id);
      expect(createdOwner.email).toBe(ownerToCreate.email);
      expect(createdOwner.password).not.toBe(ownerToCreate.password);
      // expect(createdOwner.token).toBe(data.token);
      const passwordMatched = bcrypt.compareSync(
        ownerToCreate.password,
        createdOwner.password
      );
      expect(passwordMatched).toBe(true);
    });

    test("cannot create multiple users with the same email address", async () => {
      const newUser = Object.assign({}, userToCreate);
      newUser.email += "!";
      await axios.post(`${baseUrl}/users/signup`, newUser);
      let gotError = false;
      try {
        await axios.post(`${baseUrl}/users/signup`, newUser);
      } catch (error) {
    ...
    test("sign in", async () => {
      ...
      checkLoggedInUser(loggedInUser.data.data, signInUser);
    });
    ...
    test("gets a good error message when attempting to log in with bad username and/or password", async () => {
      ...
      try {
        await axios.post(`${baseUrl}/users/login`, {
          email: "329q845gljwenpdhrefpdhury",
    ...
    test("log out", async () => {
      const userToCreate = {
        email: "userjusttologout" + Date.now() + "@logout.com",
        firstName: "FirstName" + Date.now(),
        lastName: "LastName" + Date.now(),
        password: "test.skip1234",
      };
      ...
      const { data: response } = await axios({
        ...
        headers: {
          authorization: "Bearer " + createdUser.data.data.token,
        },
      });
...
function checkLoggedInUser(userFromApi, testUser) {
  expect(typeof userFromApi.id).toBe("string");
  expect(userFromApi.email).toBe(testUser.email);
...
async function createUser(email = Math.random()) {
...
async function createOwner(email = Math.random()) {
  const result = await axios.post(`${baseUrl}/owners/signup`, {
